drop function search_saves_with_tags(uuid, text, text, int, int);

CREATE OR REPLACE FUNCTION search_saves_with_tags(
    p_user_id uuid,
...
BEGIN
    ...
    GROUP BY s.id, s.user_id, s.url, s.image_url, s.title, s.description, s.created_at
